...
import numpy as np
from DockWidget_ex import ScollWindow
...
class CanvasMainWindow(QWidget, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None,parent=None):
        ...
        self.initiateMainWindowComponents()
        ...
        self.rotateEdit = QLineEdit('0.1')
        self.rotateEdit.setFixedWidth(40)

        rotate = action('rotate', self.rotate,
                      'Ctrl+r', 'rotate')
        antirotate = action('antirotate', self.backrotate,
                      'Ctrl+r', 'antirotate')
        ...
        # Predict
        # self.predict_fields = action(get_str('predFields'), self.predict_fields_act,
        #                 'p',  enabled=True)


        # Store actions for further handling.
        # pred_fields = action(get_str('predFields'), self.predict_fields_act,
        #                      'p', 'new', get_str('crtBoxDetail'), enabled=True)
        # pred_keyvals = action(get_str('predKeyvals'), self.predict_keyvals_act,
        #                      'k', 'new', get_str('crtBoxDetail'), enabled=False)
        # crop_fields = action(get_str('cropFields'), self.crop_fields_act,
        #                      'c', 'new', get_str('crtBoxDetail'), enabled=True)
        # pred_text = action(get_str('predText'), self.predict_text_act,
        #                      't', 'new', get_str('crtBoxDetail'), enabled=False)
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                            ...
                            #   shapeLineColor=shape_line_color, shapeFillColor=shape_fill_color,
                              ...
                              zoomActions=zoom_actions,rotate=rotate,antirotate=antirotate,
                              fileMenuActions=(
                                  open,  save, save_as, close, reset_all, quit,rotate), #open_dir,
                              beginner=(), advanced=(),
                            ...
                            #   onPredictActive=(
                            #       pred_fields, crop_fields, pred_keyvals, pred_text),
        ...
        #     predict=self.menu(get_str('menu_predict')),
        #     recentFiles=QMenu(get_str('menu_openRecent')),
        #     labelList=label_menu)
        # add_actions(self.menus.predict, (
        #     pred_fields,
        #     crop_fields,
        #     pred_keyvals,
        #     pred_text))
        ...
        self.centrallayout.addWidget(scroll)
        
        self.layout.addWidget(self.mycentralwidget)
        self.filelable = QLabel('Hello')
        self.mainlayout.addWidget(self.filelable)
        
        self.mainlayout.addWidget(self.GroupBox1)

        # self.vlayout0.addWidget(self.dock)
        # self.vlayout0.addWidget(self.file_dock)
        # self.verticalGroupBox0.setMaximumWidth(200)
        # self.layout.addWidget(self.verticalGroupBox0)

        self.actions.beginner = (
            open, verify, save, #None, create, copy, delete, #open_next_image, open_prev_image, open_dir, change_save_dir, save_format, 
            None, zoom_in, zoom, zoom_out, fit_window, fit_width, None, rotate,antirotate)

        self.actions.advanced = (
            open,  save,  None, # open_next_image, open_prev_image,open_dir, change_save_dir,save_format,
        ...
        # self.loadModels()
    # def predict_fields_act(self,fieldmodel):
    #     # print('predictin fields')
    #     self.partial_reset_state()
    #     self.setCanvas()
    #     crop_images,det,names=fieldmodel.getPredictions(self.im ,self.file_path)
    ...
    def predict_keyvals_act(self):
        print('predictin fields')
            ...
            # self.canvas.set_editing(False)
            # self.actions.create.setEnabled(False)
    def initiateMainWindowComponents(self):
        horizontalGroupBox = QGroupBox("Crops")
        # self.verticalGroupBox0= QWidget() 
        self.GroupBox1 = QWidget() 
        self.mycentralwidget = QWidget() 
        # # self.widget = QWidget()  
        # self.verticalGroupBox2 = QGroupBox("Selection")
        # self.vlayout0 = QVBoxLayout()
        self.mainlayout = QVBoxLayout()
        self.centrallayout = QVBoxLayout()
        # self.verticalGroupBox0.setLayout(self.vlayout0)
        self.mycentralwidget.setLayout(self.centrallayout)
    ...
    #     xyxys=self.xywh2xyxy(yolo_boxes,self.im)
    #     # print('xyxy ',xyxy)
    #     crop_ims=[]
    #     classes=[]
    #     for xyxy,lbl in zip(xyxys,yolo_boxes[:,0]):
    #         c = int(lbl)  # integer class
    #         # print(self.label_hist[c])
    #         if  self.label_hist[c] in ['title','subtitle','field']:
    #             classes.append(self.label_hist[c])
    #             crop_ims.append(save_one_box(xyxy, np.array(self.im), BGR=True,save=False).astype(np.uint8))
        
    #     shapes=[]
    #     for cim,cls in zip(crop_ims,classes):
    #         if cls in ['title','subtitle','field']:
    #             shapes.append(cim.shape)
    #             # print(cls,cim.shape,type(cim))
    #     # plt.imshow(crop_im)
    #     crop_shapes=np.array(shapes)
    #     crop_shapes
    #     vert_sep=50
    #     hor_sep=50
    #     height,width=int((crop_shapes.mean(axis=0)[0]+vert_sep)*len(crop_shapes)),crop_shapes.max(axis=0)[1]+2*hor_sep
            
    #     newim=np.zeros((height,width,3))
    #     start_h=vert_sep
    #     start_w=hor_sep
    #     for cim,cls in zip(crop_ims,classes):
    #         if cls in ['title','subtitle','field']:
    #             h,w,_=cim.shape
    #             # print(w,h,"   ",start_w,start_w+w,start_h,start_h+h)
    #             newim[start_h:start_h+h,start_w:start_w+w,:]=cim
    #             start_h+=h+vert_sep
    #     # plt.switch_backend('TkAgg')
    #     # np.save('newim.npy',newim)
    ...
    #         yolo_boxes.append([self.label_hist.index(s.label),x_center,y_center,w,h])
    #     return yolo_boxes
    ...
    def predict_text_act(self):
        print('predictin fields')
            ...
            # self.actions.create.setEnabled(False)
    # def loadModels(self):
    #     weights=['..\\yolov5\\runs\\train\\exp20\\weights\\best.pt']
    #     self.fieldmodel=CreateDetectionModel(weights,whereto='myfields')
    ...
    def populate_mode_actions(self):
        ...
        self.tools.addWidget(self.rotateEdit)
    ...
    def save_labels(self, annotation_file_path):
        ...
        try:
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                ...
                self.label_file.save_yolo_format(annotation_file_path, shapes, self.file_path, self.image, self.label_hist,
                                                 self.line_color.getRgb(), self.fill_color.getRgb())
            elif self.label_file_format == LabelFileFormat.CREATE_ML:
    ...
    def convertPdf2Image(self,mfile,npages=1):
        images = convert_from_path(mfile,poppler_path=r'C:\Program Files\poppler-0.68.0\bin',last_page=npages)
        print('{} pages will be saved...'.format(len(images)))
        # for i in range(len(images)):   
        #     # Save pages as images in the pdf
        #     images[i].save(outfolder+file[:-4]+'_page'+ str(i) +'.jpg', 'JPEG')
        return images[0]
    def setCanvas(self):
    ...
    def load_tiffile(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            self.im = cv2.imread(unicode_file_path)

            # Use the cvtColor() function to grayscale the image

            
            # self.image_data=self.im.copy()
            # self.image_data = ImageQt(self.im)
            # self.image_data = QImage(self.im, self.im.shape[1],\
            # self.im.shape[0], self.im.shape[1] ,QImage.Format_RGB888)
            self.image_data=QImage(self.im,self.im.shape[0], self.im.shape[1] ,QImage.Format_RGB888)
            ...
            if isinstance(self.image_data, QImage):
                ...
                # image = QImage(self.image_data, self.image_data.shape[1],\
                #             self.image_data.shape[0], self.image_data.shape[1] * 3,QImage.Format_RGB888)
            # print(self.im.shape)
            if image.isNull():
            ...
            return True
        return False
    def load_pdffile(self, file_path=None):
        """Load the specified file, or the last opened file if None."""
        self.reset_state()
        self.canvas.setEnabled(False)
        if file_path is None:
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            self.im =self.convertPdf2Image(unicode_file_path)
            self.image_data = ImageQt(self.im)
    ...
    def load_file(self, file_path=None):
        ...
        if unicode_file_path and os.path.exists(unicode_file_path):
            ...
            if height>2800:
                fact=int(height/2800)
                prop_height=int(height/fact)
                pixmap3 = pixmap.scaledToHeight(prop_height)
                self.canvas.load_pixmap(pixmap3)
            else:
    ...
    def rotate(self):
        ...
        # pixmap=pixmap.translate(center.x(), center.y())
    ...
    def backrotate(self):
        pixmap = QPixmap.fromImage(self.prev_image)
    ...
    def resizeEvent(self, event):
        if self.canvas and not self.image.isNull()\
           and self.zoom_mode != self.MANUAL_ZOOM:
            self.adjust_scale()
        super(CanvasMainWindow, self).resizeEvent(event)
    ...
    def scale_fit_window(self):
        ...
        w1 = self.mycentralwidget.frameGeometry().width() - e
        h1 = self.mycentralwidget.frameGeometry().height() - e
        a1 = w1 / h1
    ...
    def scale_fit_width(self):
        ...
        w = self.mycentralwidget().frameGeometry().width() - 2.0
    ...
    def load_yolo_txt_by_filename(self, txt_path):
        ...
        print('in load_yolo_txt_by_filename shapes',shapes)
        self.load_labels(shapes)
...
def get_main_app(argv=None):
    ...
    win = CanvasMainWindow(args.image_dir,
                     args.class_file,
                     args.save_dir)
    win.show()
    return app, win
